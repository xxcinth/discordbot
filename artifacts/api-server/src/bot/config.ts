export type TicketCategory = {
  ...
  label: string;
  description: string;
  // Use a Unicode emoji string OR a custom server emoji object { id: string; name: string }
  emoji: string | { id: string; name: string };
  // The autoresponder command users should use in this ticket
...
// Role ID to ping when a ticket opens
export const STAFFIES_ROLE_ID = "1515124175264813196";

// Category channel ID where ticket channels are created
export const TICKET_CATEGORY_ID = "1521895900316696706";

// Channel ID where transcripts are sent on ticket close
export const TRANSCRIPT_CHANNEL_ID = "1515459280860479719";

// Channel ID where commission waitlist entries are posted
